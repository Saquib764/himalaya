...
(()=>{
    ...
    chrome.runtime.onMessage.addListener((message, cb)=>{
        switch(message.action) {
            ...
            case 'disconnect': 
                ...
                if(!SOCKET) return
                SOCKET.sock.close()
                connectedPeers.forEach(peer=> peer.close())
    ...
    let processingRemoteAction = false
    function setupPlayer(socket, videoEl) {
        isLocalConnected = true
        videoEl.addEventListener('play', ()=> {
            if(processingRemoteAction){
        ...
        videoEl.addEventListener('seeked', ()=> {
            if(processingRemoteAction) return
            console.log("seeked", skip.seeked)
            if(skip.seeked) return
            // socket.emit('custom_event', {
            //     to: getParticipants(),
            //     event: "doorie_seeked",
            //     payload: {
        ...
        socket.on('doorie_play', (data)=> {
            processingRemoteAction = true
            videoEl.currentTime = data.payload.currentTime+1
            videoEl.oncanplaythrough = ()=> {
                videoEl.play()
                videoEl.oncanplaythrough = null
            }
        })
        socket.on('doorie_pause', (data)=> {
            processingRemoteAction = true
            videoEl.pause()
        })
        // socket.on('doorie_seeked', (data)=> {
        //     console.log("seeked", data)

        //     skip.seeked = true
        //     skip.play = true
        //     videoEl.currentTime = data.payload.currentTime
        //     setTimeout(()=>{
        //         skip.seeked = true
        //         skip.play = true
        //         videoEl.currentTime = data.payload.currentTime + 2
        //         videoEl.play()
        //     }, 2000)
        // })
    }
    ...
    function init() {
        ...
        videoElement = document.querySelector('#dv-web-player video')
        let body = document.querySelector('div.doorie-holder')
        console.log(130, body)
        if(!body) {
            body = document.createElement('div');
            body.classList.add('doorie-holder')
            body.classList.add('doorie-top')
            let cont = document.querySelector('.cascadesContainer')
            // setTimeout(()=>{
                ...
                video.id = "doorie-my"
                let button = document.querySelector('#dv-action-box a')
                setTimeout(()=>{
                    button.click()
                }, 200)
    ...
    async function startCamera(){
        ...
        chrome.storage.sync.get('userId', (data)=> {
            ...
            SOCKET = socket
            if(socket) {
                setupPlayer(socket, videoElement)
