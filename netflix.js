...
(()=>{
    ...
    function pageScript(){
        ...
        player.addEventListener('playingchanged', ()=>{
        ...
        document.addEventListener('doorie_play_status', (e)=>{
            ...
            executingExtensionCommands ++
            if(e.detail.playing){
                player.seek(e.detail.currentTime + 1000)
                player.play()
            }else{
                player.pause()
                player.seek(e.detail.currentTime)
            }
        })
    }
    function setupPlayer(socket) {
        const script = document.createElement('script');
        script.text = `(${pageScript.toString()})();`
    ...
    function init() {
        let player = document.querySelector('#appMountPoint .sizing-wrapper')
